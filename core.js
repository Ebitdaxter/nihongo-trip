...
// più volte nello stesso giorno (bug segnalato: "nessuna carta da ripassare").
...
function startSession(kind, deck, onDone, mode, poolKind) {
  ...
  if (queue.length === 0) {
    showToast('Nessuna carta da ripassare adesso in questo mazzo. Prova un\'altra categoria/livello o torna più tardi.');
    return;
  }
  const newInThisQueue = queue.filter(c => !getSRS()[c.id]).length;
  markNewIntroduced(kind, newInThisQueue);
  currentSession = { queue, idx: 0, kind, onDone, mode: mode || 'visivo', points: 0, poolKind: poolKind || kind, currentOptions: null };
  renderSessionCard();
}
