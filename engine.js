...
var TRIP_DATE = new Date('2026-11-02T00:00:00');
var LS_PREFIX = 'sam_';
var MASTERY_DAYS = 10; // intervallo SRS (giorni) oltre il quale una carta è "padroneggiata"

// ---------------- Utility date ----------------
// Usiamo SEMPRE i componenti data in ora locale (non toISOString, che è UTC):
// altrimenti in fusi orari diversi da UTC (es. Italia, UTC+1/+2) il calcolo
// dei giorni si sfasa e le carte SRS possono ripresentarsi lo stesso giorno.
...
// Assegnati SOLO quando la risposta è corretta (quality 4). Se sbagli, niente punti.
...
// ---------------- SM-2 semplificato ----------------
// quality: 0 = sbagliato (di nuovo), 4 = corretto (bene)
...
// Numero massimo di carte per sessione (mescolate a caso ogni volta), per
// non avere sessioni infinite quando si sceglie un set/mazzo grande.
var SESSION_CAP = 30;

// Costruisce la coda di ripasso per una sessione.
// Per Vocabolario manteniamo un tetto giornaliero di carte nuove (impostabile),
// per non essere sommersi. Per Kana e Frasi NON limitiamo le carte nuove:
// sono mazzi più piccoli/finiti e l'utente deve poterli esercitare liberamente
// più volte nello stesso giorno.
// Se non c'è NULLA da ripassare/imparare (mazzo del giorno esaurito), invece di
// bloccare l'utente con un errore, offriamo comunque un giro di pratica libera
// pescando a caso dalle carte già viste: si può sempre studiare qualcosa.
var lastSessionWasFreePractice = false;
...
function buildSession(deck, kind) {
  ...
  if (kind === 'vocab') {
    ...
    newCards = fresh.slice(0, remainingNew);
  } else {
    newCards = fresh;
  }

  let queue = due.concat(newCards);
  lastSessionWasFreePractice = false;

  if (queue.length === 0 && deck.length > 0) {
    // Niente da fare oggi su questo mazzo: pratica libera su un giro casuale
    // di carte già viste, così non si resta mai bloccati.
    queue = shuffleArr(deck).slice(0, Math.min(SESSION_CAP, deck.length));
    lastSessionWasFreePractice = true;
  } else {
    queue = shuffleArr(queue).slice(0, SESSION_CAP);
  }
...
function renderDashboard() {
  ...
  el.innerHTML = `
    ...
    <div class="card score-card">
      ...
      <p class="score-hint">Ogni risposta corretta vale punti: le carte più difficili e quelle nuove ne valgono di più.</p>
    </div>
    ...
    <div class="autosave-note">${icon('check', 'autosave-icon')} I progressi si salvano da soli su questo telefono dopo ogni risposta — non serve fare nulla.</div>
  `;
}

// ================= SESSIONE QUIZ A SCELTA MULTIPLA (generica) =================
// poolKind: da dove pescare i "distrattori" (opzioni sbagliate) — indipendente
// dal filtro scelto per il mazzo di studio, così ci sono sempre abbastanza opzioni.
var currentSession = { queue: [], idx: 0, kind: null, mode: 'visivo', points: 0, poolKind: null, currentOptions: null };

function backPoolFor(poolKind) {
  if (poolKind === 'hiragana') return HIRAGANA.map(x => x.r);
  if (poolKind === 'katakana') return KATAKANA.map(x => x.r);
  if (poolKind === 'kanji') return KANJI_BASE.map(x => x.r + ' — ' + x.it);
  if (poolKind === 'vocab') return VOCAB.map(v => v.romaji + ' — ' + v.it);
  if (poolKind === 'phrases') return PHRASES.map(p => p.it);
  return [];
}
...
function buildOptionsForCard(c) {
  const fullPool = backPoolFor(currentSession.poolKind);
  const uniqueDistractors = Array.from(new Set(fullPool.filter(b => b !== c.back)));
  const chosen = shuffleArr(uniqueDistractors).slice(0, 3);
  const texts = shuffleArr([c.back].concat(chosen));
  const correctIndex = texts.indexOf(c.back);
  return { texts, correctIndex };
}

function startSession(kind, deck, onDone, mode, poolKind) {
  const queue = buildSession(deck, kind);
  if (queue.length === 0) {
    showToast('Questo mazzo è vuoto: scegli un\'altra categoria o livello.');
    return;
  }
  const newInThisQueue = queue.filter(c => !getSRS()[c.id]).length;
  markNewIntroduced(kind, newInThisQueue);
  currentSession = { queue, idx: 0, kind, onDone, mode: mode || 'visivo', points: 0, poolKind: poolKind || kind, currentOptions: null, freePractice: lastSessionWasFreePractice };
  if (lastSessionWasFreePractice) {
    showToast('Hai già ripassato tutto per oggi: eccoti un giro di pratica extra.');
  }
  renderSessionCard();
}
...
function renderSessionCard() {
  ...
  const options = buildOptionsForCard(c);
  currentSession.currentOptions = options;

  const optionsHtml = options.texts.map((t, i) =>
    `<button class="option-btn" data-i="${i}" onclick="selectOption(${i})">${t}</button>`
  ).join('');

  area.innerHTML = `
    <div class="card flash-card">
      <div class="flash-progress">${idx + 1} / ${queue.length}${currentSession.freePractice ? ' · pratica extra' : ''}</div>
      ${isAscolto
        ? `<div class="flash-front flash-front-hidden" id="flash-front-el">?</div><div class="flash-hint">Ascolta l'audio e scegli la risposta giusta</div>`
        : `<div class="flash-front" id="flash-front-el">${c.front}</div>${c.sub ? `<div class="flash-sub">${c.sub}</div>` : ''}`
      }
...
function selectOption(i) {
  ...
  if (!grid || grid.dataset.locked === '1') return;
  grid.dataset.locked = '1';

  const buttons = Array.from(grid.querySelectorAll('.option-btn'));
  buttons.forEach(b => b.classList.add('disabled'));

  const { correctIndex } = currentSession.currentOptions;
  const correct = i === correctIndex;
  buttons[correctIndex].classList.add('correct');
  if (!correct) buttons[i].classList.add('incorrect');

  const c = currentSession.queue[currentSession.idx];

  // rivela il carattere/frase se era nascosto (modalità ascolto)
  ...
  if (correct) {
    ...
    showToast('Corretto! +' + pts + ' punti');
  } else {
    showToast('Risposta sbagliata — la trovi qui sopra evidenziata.');
  }
...
// ================= KANA =================
var KANA_MODE_HELP = {
  visivo: 'Guarda il carattere e scegli la lettura corretta tra le opzioni proposte.',
  ascolto: 'Il carattere resta nascosto: ascolta l\'audio e scegli la risposta giusta tra le opzioni. Il carattere verrà rivelato subito dopo. Allena l\'orecchio.',
};
    ...
    <div class="card">
      ...
      <div class="row-controls">
        <select id="kana-mode" onchange="updateKanaModeHelp()">
          <option value="visivo">Modalità: Visivo (leggi e scegli)</option>
...
function launchKanaSession() {
  const set = document.getElementById('kana-set').value; // hiragana | katakana | kanji
  const mode = document.getElementById('kana-mode').value;
  let deck;
  if (set === 'kanji') {
    const grp = document.getElementById('kana-grp').value;
  ...
  startSession('kana', deck, () => { renderKanaHome(); }, mode, set);
}
...
function launchVocabSession() {
  const cat = document.getElementById('vocab-cat').value;
  const deck = buildDeck('vocab', cat ? { cat } : {});
  startSession('vocab', deck, renderVocabHome, 'visivo', 'vocab');
}
...
function renderPhrasesHome() {
  ...
  el.innerHTML = `
    <div class="card">
      ...
      <p class="muted">Ascolta ogni frase, ripetila ad alta voce imitando ritmo e intonazione, poi rispondi al quiz. Stessa frase per 3-5 giorni prima di passare oltre.</p>
      <div class="row-controls">
...
function launchPhraseSession() {
  const cat = document.getElementById('phrase-cat').value;
  const deck = buildDeck('phrases', cat ? { cat } : {});
  startSession('phrases', deck, renderPhrasesHome, 'visivo', 'phrases');
}
...
function renderSettings() {
  ...
  el.innerHTML = `
    ...
    <div class="card">
      <h3>Impostazioni</h3>
      <label class="field-label">Nuove carte al giorno (solo Vocabolario)</label>
